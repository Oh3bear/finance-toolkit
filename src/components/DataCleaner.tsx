                ...
                {Array.from({ length: displayCols }).map((_, ci) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {hasRule && !isSelected && (
                        <Badge className="mt-0.5 h-4 text-[9px] px-1 bg-primary/50 hover:bg-primary/50">
  ...
  const renderRulePanel = () => {
    ...
    return (
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-2 px-2 py-1.5 bg-primary/5 rounded-lg border border-primary/20">
            ...
            <div className="flex flex-wrap gap-1">
              {colRules.map(rule => (
                <Badge key={rule.id} variant="outline" className="text-[10px] h-5 px-1">
                  ...
                  <button onClick={() => handleDeleteRule(rule.id)} className="ml-1 hover:text-destructive">
              ...
              <tbody>
                {display.map((r, i) => (
                  <tr key={i} className={r.changed ? 'bg-amber-50/50' : ''}>
                    ...
                    <td className={`px-3 py-1 max-w-[200px] truncate ${r.changed ? 'text-primary font-medium' : 'text-muted-foreground'}`}>
                      {r.cleaned || '(空)'}
                    </td>
  ...
  const renderBatchSection = () => {
    ...
    return (
      <div className="space-y-3">
        ...
        {/* Batch upload */}
        <div
          className="border-2 border-dashed border-border rounded-xl py-10 px-6 text-center cursor-pointer
          ...
          onDragOver={e => e.preventDefault()}
        >
          <div className="w-12 h-12 bg-gradient-to-br from-primary/10 to-primary/5 rounded-xl flex items-center justify-center mx-auto mb-3">
            <Upload className="w-6 h-6 text-primary/70" />
          </div>
          <p className="text-sm text-foreground font-medium">上传待处理的 Excel 文件</p>
          <p className="text-xs text-muted-foreground mt-1">支持多选 · 支持 .xlsx / .xls 格式</p>
          <input
            ref={batchInputRef}
            type="file"
            accept=".xlsx,.xls"
          ...
          <div className="space-y-1 max-h-[150px] overflow-y-auto">
            {batchFiles.map(bf => (
              <div key={bf.id} className="flex items-center gap-2 px-3 py-1.5 bg-card rounded border border-border text-xs">
                ...
                <button onClick={() => setBatchFiles(prev => prev.filter(f => f.id !== bf.id))} className="text-muted-foreground hover:text-destructive shrink-0">
        ...
        <div className="border border-border rounded-lg overflow-hidden bg-card">
          <div className="overflow-auto max-h-[300px]">
            <table className="w-full text-xs">
              ...
              <tbody>
                {results.map((r, i) => (
                  <tr key={i} className={r.status === 'error' ? 'bg-destructive/5' : 'hover:bg-background'}>
