...
const EXTRACT_STEPS: { id: ExtractStep; label: string; icon: React.ReactNode }[] = [
  ...
  { id: 'batch', label: '批量提取', icon: <Download className="w-3.5 h-3.5" /> },
];
      ...
      <div className="bg-card border-b border-border/60 sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-4 py-3">
          <ForgeStepIndicator
            steps={EXTRACT_STEPS}
            currentStep={step}
            onStepClick={(id) => setStep(id as ExtractStep)}
            canNavigateTo={(id) => {
            ...
            <div className="bg-card rounded-2xl border border-border p-6 space-y-6">
              <div>
                <h2 className="text-lg font-bold text-foreground flex items-center gap-2">
                  <Upload className="w-5 h-5" style={{ color: 'hsl(210,90%,56%)' }} />
                  上传样本
                </h2>
        ...
        {step === 'config' && sampleFile && (
          <div className="space-y-4">
            <div className="bg-card rounded-2xl border border-border p-6 space-y-6">
              <div>
                <h2 className="text-lg font-bold text-foreground flex items-center gap-2">
                  <Settings2 className="w-5 h-5" style={{ color: 'hsl(210,90%,56%)' }} />
                  配置提取规则
                </h2>
                <p className="text-sm text-muted-foreground mt-1">
        ...
        {step === 'batch' && sampleFile && (
          <div className="space-y-4">
            <div className="bg-card rounded-2xl border border-border p-6 space-y-6">
              <div>
                <h2 className="text-lg font-bold text-foreground flex items-center gap-2">
                  <Download className="w-5 h-5" style={{ color: 'hsl(210,90%,56%)' }} />
                  批量处理
                </h2>
                <p className="text-sm text-muted-foreground mt-1">
