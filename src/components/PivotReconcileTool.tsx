...
function DropZone({
  ...
  if (file) {
    return (
      <div className="flex items-center justify-between gap-3 p-4 rounded-xl border border-border/60" style={{ background: 'hsl(210,90%,56%,0.04)' }}>
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0" style={{ background: 'hsl(210,90%,56%,0.1)' }}>
            <FileSpreadsheet className="w-5 h-5" style={{ color: 'hsl(210,90%,56%)' }} />
          </div>
  ...
  return (
    <div
      className={`saas-upload-zone rounded-xl p-12 text-center cursor-pointer transition-all duration-300 ${
        dragging
          ? 'active'
          : ''
      }`}
...
const STEPS: { id: Step; label: string; icon: React.ReactNode }[] = [
  { id: 'upload-pivot', label: '上传二维表', icon: <Table2 className="w-3.5 h-3.5" /> },
  { id: 'config-pivot', label: '配置二维表', icon: <ArrowUpDown className="w-3.5 h-3.5" /> },
  { id: 'upload-flat', label: '上传一维表(可选)', icon: <Table className="w-3.5 h-3.5" /> },
  { id: 'config-flat', label: '配置一维表', icon: <ArrowUpDown className="w-3.5 h-3.5" /> },
  { id: 'reconcile', label: '执行核对', icon: <Play className="w-3.5 h-3.5" /> },
  { id: 'result', label: '核对结果', icon: <CheckCircle2 className="w-3.5 h-3.5" /> },
];
...
export default function PivotReconcileTool() {
    ...
    <Card className="max-w-lg mx-auto">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          <Table2 className="w-5 h-5" style={{ color: 'hsl(210,90%,56%)' }} />
          上传二维交叉表
        </CardTitle>
                ...
                {headerRow.map((h: any, i: number) => {
                  ...
                  return (
                    <button
                      ...
                      style={isSelected ? {
                        background: 'hsl(210,90%,56%,0.08)',
                        borderColor: 'hsl(210,90%,56%,0.3)',
                        color: 'hsl(210,90%,56%)',
                      } : undefined}
                    >
                  ...
                  <thead>
                    <tr className="bg-muted/50">
                      {flatTable.headers.map((h, i) => (
                        ...
                        style={flatRowKeyIndices.includes(i) ? { color: 'hsl(210,90%,56%)' } : undefined}>
                          {h || `列${i + 1}`}
                        </th>
                      ))}
                    </tr>
  ...
  const renderReconcile = () => (
    <Card className="max-w-lg mx-auto">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          <RotateCw className="w-5 h-5" style={{ color: 'hsl(210,90%,56%)' }} />
          执行逆透视{skipFlat ? '' : '与核对'}
        </CardTitle>
        <p className="text-sm text-muted-foreground">
          {skipFlat
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-muted/30 rounded-xl p-4 space-y-2 text-sm">
          <div className="flex items-center gap-2">
            <Table2 className="w-4 h-4" style={{ color: 'hsl(210,90%,56%)' }} />
        ...
        <div className="flex justify-between">
          ...
          <Button onClick={executeReconcile} className="gap-1.5">
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2">
              <CheckCircle2 className="w-5 h-5" style={{ color: 'hsl(210,90%,56%)' }} />
              核对结果摘要
            </CardTitle>
          </CardHeader>
          <CardContent>
